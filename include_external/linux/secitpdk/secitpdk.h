...
//设置结构体信息回调函数（与SECITPDK_SetMsgCallback内容相同，只是格式不同，一个是结构体格式，一个是JSON串格式，二选一注册即可，两个都注册时仅推送结构体）
ITPDK_API void SECITPDK_SetStructMsgCallback(pStructMessageCallbackMethod func);

// 异步回调函数（消息数据pMsg为json串）
ITPDK_API void SECITPDK_SetFuncCallback(pAsyncCallbackFunc func);

// 设置普通买卖接口 结构体异步回调函数（做普通买卖异步委托时，与SECITPDK_SetFuncCallback内容相同，只是格式不同，一个是结构体格式，一个是JSON串格式，二选一注册即可，两个都注册时仅推送结构体；若做其他业务的异步委托，该函数无效，需要调用SECITPDK_SetFuncCallback）
ITPDK_API void SECITPDK_SetStructOrderFuncCallback(pStructOrderAsyncCallbackFunc func);
...
/*
    ...
    khh         [in]        客户号
    nType       [in]        是否仅查询可撤/仅查询委托（0查询全部，1查询可撤，2仅查委托，3仅查撤单）
    ...
    arDrwt      [out]        结果数据
	nKFSBDBH	[in]		开发商本地编号（为空时查询所有）
    return              返回查询到的记录数
*/
ITPDK_API int64 SECITPDK_QueryOrders(const char* khh, int nType, int nSortType, int nRowcount, int64 nBrowindex, const char* jys, const char* zqdm, int64 lWth, vector<ITPDK_DRWT>& arDrwt, int64 nKFSBDBH = 0);
ITPDK_API int64 SECITPDK_QueryOrdersEx(ITPDK_CusReqInfo& cusreqinfo, int nType, int nSortType, int nRowcount, int64 nBrowindex, const char* jys, const char* zqdm, int64 lWth, vector<ITPDK_DRWT>& arDrwt, int64 nKFSBDBH = 0);
...
/*
    ...
    lpBz  		  [in]        币种
    sZqdm         [in]        证券代码
    nLogical      [in]        查询逻辑 (0：按证券代码排序 1：按股东号排序 2:按证券代码和交易类别排序)
    nFlag         [in]        是否返回撤单记录(0 返回,1 不返回 2同时不返回废单记录)
    arSSCJHZ      [out]        结果数据
    return              返回查询到的记录数
*/
ITPDK_API int64 SECITPDK_QuerySSCJHZ(const char* lpKhh, const char* lpJszh, const char* lpGdh, const char* lpJys, const char* lpBz, const char* sZqdm, int nLogical, int nFlag, vector<ITPDK_SSCJHZ>& arSSCJHZ);
ITPDK_API int64 SECITPDK_QuerySSCJHZEx(ITPDK_CusReqInfo& cusreqinfo, const char* lpJszh, const char* lpGdh, const char* lpJys, const char* lpBz, const char* sZqdm, int nLogical, int nFlag, vector<ITPDK_SSCJHZ>& arSSCJHZ);
...
/*
    ...
    return              成功返回结果数，失败返回<0
*/
ITPDK_API int64 SECITPDK_QueryJYJWInfo(const char* khh, const char* lpJys, const char* lpZqdm, const char* lpZQLB, vector<ITPDK_JYJW>& arJYJW);
ITPDK_API int64 SECITPDK_QueryJYJWInfoEx(ITPDK_CusReqInfo& cusreqinfo, const char* lpJys, const char* lpZqdm, const char* lpZQLB, vector<ITPDK_JYJW>& arJYJW);
...
/*
    ...
    khh         [in]        客户号
	jys			[in]		交易所（为空时查询所有交易所）
	type		[in]		0：实物申赎，1：现金申赎，为空时查询所有
	sgshzt      [in]        申购申赎状态（需依赖后台插件升级，目前仅东吴可用）
    arEtfxx     [out]       结果数据
    return                成功返回结果数，失败返回<0
*/
ITPDK_API int64 SECITPDK_QueryALLETFXX(const char* khh, const char* jys, const char* type, const char* sgshzt, vector<ITPDK_ETFXX>& arEtfxx);
ITPDK_API int64 SECITPDK_QueryALLETFXXEx(ITPDK_CusReqInfo& cusreqinfo, const char* jys, const char* type, const char* sgshzt, vector<ITPDK_ETFXX>& arEtfxx);
/*
    查询ETF明细，成分股信息
...
/*
	...
	return                  成功返回>0，失败返回<0
*/
ITPDK_API int64 SECITPDK_BankBusiness_FK(ITPDK_REQ_YZZZ &pReqYZZZ, char *sSQH);
...
/*
    ...
    lDdlx       [in]        订单类型(可在itpdk_dict.h中查看宏定义，格式为DDLX_*)
    lpGdh       [in]        股东号
...
/*
	...
	return                  小于0：表示非交易错误或其他导致无法进行委托的错误，可通过SECITPDK_GetLastError获取错误信息
						其他：表示买卖失败的数量，可通过参数获取失败信息
*/
ITPDK_API int64 SECITPDK_BatchOrderEntrustMod(const char* lpKhh, vector<BatchOrderInfoMod>& arBatOrder, int64 nWTPCH);
ITPDK_API int64 SECITPDK_BatchOrderEntrustModEx(ITPDK_CusReqInfo& cusreqinfo, vector<BatchOrderInfoMod>& arBatOrder, int64 nWTPCH);

/*
	异步可指定KFSBDBH普通交易批量委托
	...
	return              成功返回>0
*/
ITPDK_API int64 SECITPDK_BatchOrderEntrustMod_ASync(const char* lpKhh, vector<BatchOrderInfoMod>& arBatOrder, int64 nWTPCH);
ITPDK_API int64 SECITPDK_BatchOrderEntrustModEx_ASync(ITPDK_CusReqInfo& cusreqinfo, vector<BatchOrderInfoMod>& arBatOrder, int64 nWTPCH);
...
/*
    ...
    lpJys        [in]        交易所
    lpZqdm       [in]        证券代码
    nJylb        [in]        交易类别(JYLB_JJRG基金认购,JYLB_JJSG基金申购,JYLB_JJSH基金赎回,JYLB_JJFC基金分拆,JYLB_JJHB基金合并,JYLB_JJFHSZ基金分红设置)
    lWtsl        [in]        委托数量
...
/*
	...
	return                成功返回>0，失败返回<0
*/
ITPDK_API int64 SECITPDK_BatchOrderWithdrawMod(const char* lpKhh, int64 nLSH);
ITPDK_API int64 SECITPDK_BatchOrderWithdrawModEx(ITPDK_CusReqInfo& cusreqinfo, int64 nLSH);
...
/*
	...
	return                成功返回开发商本地编号，失败返回<0
*/
ITPDK_API int64 SECITPDK_BatchOrderWithdrawMod_ASync(const char* lpKhh, int64 nLSH);
ITPDK_API int64 SECITPDK_BatchOrderWithdrawModEx_ASync(ITPDK_CusReqInfo& cusreqinfo, int64 nLSH);
...
/*
    ...
    nJhm         [in]        激活码
	nType		 [in]		 激活类型（0：发往交易所，1：发往中登）
    return              成功返回大于0，失败返回<=0
*/
ITPDK_API int64 SECITPDK_PasswordService(const char* lpKhh, const char* lpGdh, const char* lpJys, int nJylb, int64 nJhm, int nType = 0);
ITPDK_API int64 SECITPDK_PasswordServiceEx(ITPDK_CusReqInfo& cusreqinfo, const char* lpJys, int nJylb, int64 nJhm, int nType = 0);
...
/*
    ...
    return              返回查询到的记录数
*/
ITPDK_API int64 SECITPDK_QueryZQGL_MM(const char* lpKeyName, const char* khh, int jdbh, int nSortType, int nRowcount, int64 nBrowindex, const char* gdh, const char* jys, const char* zqdm, int32 exeflag, vector<ITPDK_ZQGL>& arZqgl, const char *sGyh, const char *sGymm);
...
/*
    ...
    tgdy        [in]        托管单元
    gfxz        [in]        股份性质
    ldjsl       [in]        冻结数量（itpdk端均为正数）
    sGyh        [in]        柜员号
    sGymm       [in]        柜员密码
    llsclms     [in]        流水处理模式（如果等于1，将不记录冻结流水，不检查发生数量不能超过ZQSL; 等于2，记录冻结流水，不检查发生数量不能超过ZQSL）
    czbz        [in]        操作标志（0-冻结/1-解冻）
    ylbzw       [in]        预留标志位
    return              成功返回>=0，失败返回<0
*/
ITPDK_API int64 SECITPDK_FreezeStock_MM(const char* lpKeyName, const char* khh, int jdbh, const char* gdh, const char* jys, const char* zqdm, const char* ltlx, const char* tgdy, const char* gfxz, int64 ldjsl, const char *sGyh, const char *sGymm, int64 llsclms, int nCzbz, const char * sYlbzw);
...
/*
    ...
    ylbzw       [in]        预留标志位
    return              
       冻结：返回值>0时表示功能执行成功,返回值为冻结流水号，返回值<0时表示功能执行失败
       解冻：返回值=0时表示解冻操作成功，返回值<0时，表示功能执行失败
*/
ITPDK_API int64 SECITPDK_Yzyzgb_FreezeUnfreeze(const char* lpKeyName, const char* khh, const char* gdh, int jdbh, const char* zqdm, const char* jys, int64 lLsh, int nCzbz, int64 lFssl, const char *sGyh, const char *sGymm, const char * sZY, const char * sYlbzw);

/*
    大宗交易（中信）
    sKHH        [in]        客户号
    lJYLB       [in]        交易类别（JYLB_YXMR意向买入,JYLB_YXMC意向卖出,JYLB_DJMR定价买入,JYLB_DJMC定价卖出,JYLB_MRQR买入确认,JYLB_MCQR卖出确认,JYLB_PHMR盘后买入,JYLB_PHMC盘后卖出）
    sJYS        [in]        交易所
    sZQDM       [in]        证券代码
    sGDH        [in]        股东号
    lWTSL       [in]        委托数量
    ...
    return                成功返回委托号，失败返回<0
*/
ITPDK_API int64 SECITPDK_DZOrderEntrust(const char* sKHH, int64 lJYLB, const char* sJYS, const char* sZQDM, const char* sGDH, int64 lWTSL, double lfWTJG, int64 lDDLX, const char* sDFGDH, const char* sDFXW, const char* sYDH, const char* sKZLSH, const char* sKFSDM, int64 nWTPCH, int nPDDM, int nTDBZ, const char* sLXDH, const char* sLXR);
...
/*
    ...
    sLXDH       [in]        联系电话
    sLXR        [in]        联系人
    nKFSBDBH    [in]        第三方开发商报单编号
    return                成功返回委托号，失败返回<0
*/
ITPDK_API int64 SECITPDK_DZOrderEntrust_ASync(const char* sKHH, int64 lJYLB, const char* sJYS, const char* sZQDM, const char* sGDH, int64 lWTSL, double lfWTJG, int64 lDDLX, const char* sDFGDH, const char* sDFXW, const char* sYDH, const char* sKZLSH, const char* sKFSDM, int64 nWTPCH, int nPDDM, int nTDBZ, const char* sLXDH, const char* sLXR, int64 nKFSBDBH = 0);
...
/*
    ...
    return                成功返回>0，失败返回<0
*/
ITPDK_API int64 SECITPDK_DZOrderWithdraw(const char* sKHH, const char* sJYS, int64 nCxwth);
...
/*
    ...
    return                失败返回<0，成功返回KFSBDBH
*/
ITPDK_API int64 SECITPDK_DZOrderWithdraw_ASync(const char* sKHH, const char* sJYS, int64 nCxwth, int64 nKFSBDBH = 0);
...
/*
    ...
    return                成功返回>0，失败返回<0
*/
ITPDK_API int64 SECITPDK_NBJQHQ(const char* sJRKHH, const char* sJRGDH, int64 nCZLB, const char* sJCKHH, const char* sJCGDH, const char* sJYS, const char* sZQDM, int64 nFSSL);
...
/*
    ...
    sGymm       [in]        柜员密码
    jsrq        [in]        交收日期
    czbz        [in]        操作标志（0-资金额度调入/1-资金额度调出）
    ylbzw       [in]        预留标志位
    return              成功返回>=0，失败返回<0
*/
ITPDK_API int64 SECITPDK_HTSNBKYZJ(const char* lpKeyName, const char* khh, int jdbh, const char* zjzh, const char* bz, double fsje, const char *sGyh, const char *sGymm, int64 jsrq, int nCzbz, const char * sYlbzw);

/*
	查询集中度占比（中信）

    lpKeyName   [in]        使用的配置Key值
    nType       [in]        查询字段区分（0查询ZGBZB字段，1查询QYCCHBZB字段）
	sKhh        [in]        客户号
    jdbh        [in]        节点编号
    nRowcount   [in]        返回条数
    nBrowindex  [in]        分页
	...
	return              返回查询到的记录数
*/
ITPDK_API int64 SECITPDK_QueryZGBZB(const char* lpKeyName, int nType, const char* sKhh, int jdbh, int nRowcount, int64 nBrowindex, const char* sJys, const char* sZqdm, const char* sZqlb, const char *sGyh, const char *sGymm, vector<ITPDK_ZGBZB>& arZgbzb);

/*
	修改集中度占比（中信）

	lpKeyName   [in]        使用的配置Key值
    nType       [in]        查询字段区分（0修改ZGBZB字段，1修改QYCCHBZB字段）
	sKhh        [in]        客户号
	jdbh        [in]        节点编号
	sJys		[in]        交易所
	sZqdm       [in]        证券代码
	...
	return              成功返回>0，失败返回<0
*/
ITPDK_API int64 SECITPDK_ModifyZGBZB(const char* lpKeyName, int nType, const char* sKhh, int jdbh, const char* sJys, const char* sZqdm, double dZGBZB, const char *sGyh, const char *sGymm);
...
/*
    ...
    tgdy        [in]        托管单元
    gfxz        [in]        股份性质
    lzqsl       [in]        股份数量（itpdk端均为正数）
    sGyh        [in]        柜员号
    sGymm       [in]        柜员密码
    czbz        [in]        操作标志（0-转入/1-转出）
    zgbzbjc     [in]        检查总股本占比（0-不检查/1-检查）
    zy          [in]        摘要
    ylbzw       [in]        预留标志位
    return              成功返回>=0，失败返回<0
*/
ITPDK_API int64 SECITPDK_GFZRZC_MM(const char* lpKeyName, const char* khh, int jdbh, const char* gdh, const char* jys, const char* zqdm, const char* ltlx, const char* tgdy, const char* gfxz, int64 lzqsl, const char *sGyh, const char *sGymm, int nCzbz, int nZgbzbjc, const char * sZY, const char * sYlbzw);
...
/*
    ...
    return              返回查询到的记录数
*/
ITPDK_API int64 SECITPDK_QueryZJXX_MM(const char* lpKeyName, const char* khh, int jdbh, vector<ITPDK_ZJZH>& arZjzh, const char *sGyh, const char *sGymm);
...
/*
    ...
    sJymm        [in]        交易密码
    nRowcount   [in]        返回条数
    nBrowindex  [in]        分页定位串（按流水号）
	arZQDBMX	[out]		证券调拨明细
    return                成功返回>=0，失败返回<0
*/
ITPDK_API int64 SECITPDK_StockTransDetail(const char* sKhh, const char* sJymm, int nRowcount, const char* lpBrowindex, vector<ITPDK_ZQDBMX>& arZQDBMX);
...
/*
    ...
    return                成功返回>=0，失败返回<0
*/
ITPDK_API int64 SECITPDK_QueryGTCCXX(const char* sKhh, const char* sJymm, vector<ITPDK_GTCCXX>& arGTCCXX);
/*
    股票期权备兑证券锁定/解锁股份冻结
    ...
    lFssl       [in]        发生数量
    nFlag       [in]        0=锁定，1=解锁
    return                成功返回锁定流水号，失败返回<0
*/
ITPDK_API int64 SECITPDK_SOPStockLock(const char* lpKeyName, const char* lpKhh, const char* gdh, const char* zqdm, const char* jys, int64 fssl, bool flag, const char* sGyh, const char* sGymm);    //锁定/解锁
/*
    股票期权可加解锁数量计算
    ...
    return                成功返回数量，失败返回<0
*/
ITPDK_API int64 SECITPDK_SOPStockLockAmount(const char* lpKeyName, const char* khh, const char* gdh, const char* zqdm, const char* jys, int64 flag, const char* sGyh, const char* sGymm);      //可加解锁数量>>>>>>> .merge-right.r90435
