...
{
    char        AccountId[13];          //客户号
    int32       SystemType;             //系统类型
    uint8       EntrustType;            //交易类型
    char        KFSBDBH[17];
    int64       OrderId;                //委托号
    char        OrderTime[13];          //委托时间
    float64     FrozenBalance;          //冻结资金
    int16       nRetCode;               //返回值
    char        sRetNote[128];          //返回结果说明
	char		DevelCode[7];			//开发商代码

    stStructOrderFuncMsg()
    {
        memset(this, 0, sizeof(stStructOrderFuncMsg));
    }
};
typedef void(*pStructOrderAsyncCallbackFunc)(const char* pTime, stStructOrderFuncMsg &stMsg, int nType);
...
typedef struct
{
    ...
    char     HolderName[80];   //股东姓名
    char     FundAccount[16];  //资金帐号
    char     OrgCode[8];       //机构代码--结算银行
    char     MoneyType[4];     //币种
    char     TradeAccess[128];  //交易权限
    int      HolderType;       //股东类别
...
struct BatchOrderInfo
{
    ...
    double  Wtjg;       //委托价格
    int64   Wtsl;       //委托数量
    int     Ddlx;       //订单类型(可在itpdk_dict.h中查看宏定义，格式为DDLX_*)
    char    Gdh[11];
...
struct BatchOrderInfoMod
{
	...
	int64	kfsbdbh;	//开发商本地编号(范围在0~9999999999999999之间，最大16位)
	char    Gdh[11];
