...
#define ID_TKTYPE_NBSTEP		9		//新债券逐笔成交, 深交所新债券
#define ID_TKTYPE_NBORDER		10		//新债券逐笔委托, 深交所新债券
...
// 使用NonMD非行情快照的标志
#define MD_FLAGS_NONMD		0x8000

// 使用OTC快照的标志
#define MD_FLAGS_OTCOP		0x0100

// 使用BBQ快照的标志
#define MD_FLAGS_BBQMD		0x0200
...
{	
    ...
    struct TD_Info_Bond
    {
		...
		uint8_t  nPricingMethod;	 //1=净价  2=全价
	...
	struct TD_Info_Options				//2013.10.23 add by Paul for ShangHai Option
	{
		...
		char chContractID2[48];         // 期权合约代码C40, 长名称
	}Options;
};
...
union TD_AB_Ext
{
    ...
    {
        int64_t nSettlement;    // 结算价
        int nPositions;     // 持仓量
        int nCurDelta;      // 虚实度
        int64_t nUnderlingPrice;// 标的品种价格
    }Futures;
	struct TD_AB_Bonds
	{
		int64_t nSettlement;    // 结算价
...
union TD_Exdata
{
	struct TD_Exdata_Shares
	{
		...
		int nTime;              //盘前/盘后时间		
		uint32_t nBeforeVolume;  //盘前量
		uint32_t nAfterVolume;	//盘后量
	...
	struct TD_Exdata_Bond
	{
		...
		int64_t iAuctionVolumeTrade;  //匹配成交成交量
		int64_t iAuctionValueTrade;	  //匹配成交成交金额
		int64_t iPriceAuctionTrade;	  //匹配成交最近价
		char	chTradingPhaseCode[5];//交易方式所处的交易阶段代码:    N0=匹配成交 N1=协商成交 N2=点击成交 N3=询价成交 N4=竞买成交
		uint8_t	uTradingType;		  //当前最新价对应的交易方式:	1=匹配成交 2=协商成交 3=点击成交 4=询价成交 5=竞买成交
		int16_t nTypeID;			  //行情类别, 深交所对应MDStreamID的整数
		int64_t iPriceOpen;			  //非匹配类型开盘价
		int64_t iPriceHigh;			  //非匹配类型最高价
		int64_t iPriceLow;	          //非匹配类型最低价
		int64_t iReserved1;
		int64_t iReserved2;
		int64_t iReserved3;
	}Bond;
	...
	{
		uint32_t nPreSettlement32;	// 前日结算价, 32位
		int nPrePosition;  // 前日持仓量
		int nPreDelta;     // 前日虚实度
		uint32_t nAvgPrice32;	   //均价, 32位
		uint32_t nDayVolume;    //日间量
		uint32_t nNightVolume;  //夜间量
		int64_t iDayPrice;     //日间价格
		int64_t iNightPrice;   //夜间价格	
		int64_t iEFPVolume;
		int     nEFPPositions; 
		int64_t nPreSettlement;		// 前日结算价
		int64_t nAvgPrice;			// 均价
	}Futures;
	struct TD_Exdata_FX
	{
	...
	struct TD_Exdata_IR_SharesLend
	{
		int64_t iLendAmount;		//融出数量
		int64_t iBorrowAmount;		//借入数量
		int		nBestLendRate;		//最低融出利率 * 10000
		int		nBorrowRate;		//借入利率 * 10000
		int64_t iPreTradeVolume;	//昨日成交量
		int64_t iPreTradeMoney;		//昨日成交额*10000
		char	chLendTerms[12];	//融出期限
		char	chBorrowTerms[12];	//借入期限
	}IRSharesLend;
	struct TD_Exdata_OR_FutureMS
	{
		short	sFundFlow[22];		//资金流向数据
	}ORFutureMS;					//对应ID_BT_OTHER_FUTUREMS类型
};
...
union TD_Exdata2
{
	struct TD_Exdata_SH
	{
		int nWithdrawBuyNumber;			// 买入撤单笔数
		int nWithdrawSellNumber;		// 卖出撤单笔数
		int64_t iWithdrawBuyAmount;		// 买入撤单数量
		int64_t iWithdrawBuyMoney;      // 买入撤单金额		
		int64_t iWithdrawSellAmount;		// 卖出撤单数量
		int64_t iWithdrawSellMoney;      // 卖出撤单金额
		int nTotalBidNumber;			// 买入总笔数
		int nTotalOfferNumber;			// 卖出总笔数
		int nBidTradeMaxDuration;		// 买入委托成交最大等待时间
		int nOfferTradeMaxDuration;		// 卖出委托成交最大等待时间
		int nNumBidOrders;				// 买方委托价位数
		int	nNumOfferOrders;			// 卖方委托价位数
	}SH_Exdata; 
};
...
struct TD_Order_Packet
{
    ...
    int  nOrder;         // 委托序号
	int	 nChannel;		 // channel id
	char chOrderKind;    // 委托类别	(上交所新债券，新增类型‘S’, 表示逐笔订单状态)
	char chFunctionCode; // 委托代码	(如果chOrderKind为‘S’, 则本字段表示具体状态，有'Y''E' 'I''O''X''C''V'; 含义可参见TDMarketData_Packet中的chStatusFlag的说明)
	char chStatus;       // 委托状态
	char chFlag;         // 标志	
...
//新债券逐笔委托///////////////////////////////////////////
struct TD_NBOrder_Packet
{
	struct TD_Order_Packet	order;

	int		nTypeID;				//行情类别, 深交所对应MDStreamID的整数
	uint8_t uSettlPeriod;			//结算周期
	uint8_t	uSettlType;				//结算方式
	uint8_t	uBidTransType;			//竞买业务类别
	uint8_t	uBidExecInstType;		//竞买成交方式
	...
	char	chMemo[164];			//备注
	int64_t	nHighLimitPrice;		//价格上限
	int64_t	nLowLimitPrice;			//价格下限
	int64_t	nMinQty;				//最低成交数量
	int		nTradeDate;				//交易日期
};

//新债券逐笔成交///////////////////////////////////////////
struct TD_NBStep_Packet
{
	struct TD_Step_Packet	step;
	...
	uint8_t	uBidExecInstType;		//竞买成交方式
	char	chSecondaryOrderID[16];	//竞买场次编号
	int64_t	nMarginPrice;			//达成成交的边际价格
	int64_t iNetPrice;				//成交净价
	int64_t iFullPrice;				//成交全价
	int64_t iAccruedInterest;   	//应付利息
	int64_t iYieldToMaturity;       //到期收益率
	int64_t iTradeAmount;			//成交金额
};

//#BBQ 盘口
struct TD_BBQEX_ASKBID
{ 
	int nTime;                  //报价时间，单位为秒
	int64_t iPrice;				//价格，原始值*10000
	int64_t iYield;				//收益率,原始值*10000
	int64_t iDirtyPrice;		//全价,原始值*10000
	int64_t iNetPrice;			//净价,原始值*10000
	int64_t iVolumeTotal;		//报价总量
	char chQuoteId[64];         //报价ID
	char chPriceDesc[64];		//报价描述
	char chMemo[512];			//报价原始备注
	char chVolumeValue[120];	//报价量描述
	char chPriceType;			//报价类型
	uint8_t uExeciseFlag;       //行权标志
	uint8_t uBargainFlag;		//可议价标志
	uint8_t uRelationFlag;		//打包和OCO标志
};

struct TD_BBQEX_INDEX
{
	int nTime;					//时间，单位为秒
...
struct TD_BBQEx_Snap
{
	...
	TD_BBQEX_ASKBID stAsk;      //卖报价信息
	TD_BBQEX_ASKBID stBid;      //买报价信息
	TD_BBQEX_INDEX  stIndex;	//Index
};

//BBQ逐笔成交///////////////////////////////////////////
struct TD_BBQEx_Deal
{
	int nType;						//类型
	int nSourceCode;				//中介机构：国际001，国利002，中诚003，平安004，信唐005，上田006
	int nDate;						//交易日期，年月日
	...
	int64_t iTradePrice;			//成交价,原始值*10000
	int64_t iYieldRate;				//成交收益率,原始值*10000
	int64_t iNetPrice;				//成交净价,原始值*10000
	int64_t iDirtyPrice;			//成交全价,原始值*10000
	int64_t iVolume;				//成交量	
	char chSettlSpeed[56];			//清算速度
	char chPriceType;				//成交价类型  1：净价；2：全价；3：收益率
	uint8_t uExerciseFlag;			//行权标志  若存在“行权”字样则赋值为0   若存在“到期”字样则赋值为1
	uint8_t uStatus;                //成交类型 1:done  2:gvn  3:tkn  4:默认  6:ref撤销
	uint8_t uDelFlag;				//删除标识  0:正常  1:删除    
};


struct TD_FixedIncome_BondHQ
{
	char		chBidQuoteOrg[32];   //买报价方
	char		chAskQuoteOrg[32];   //卖报价方
	int64_t     iBidCleanPrice;      //最优买入净价
	int64_t     iAskCleanPrice;      //最优卖出净价
	int			nBidYield;           //最优买入到期收益率
	int			nAskYield;           //最优卖出到期收益率
	int64_t		iBidTradeVolume;     //最优买入报价量
	int64_t		iAskTradeVolume;     //最优卖出报价量
	int64_t     iBidTotalPrice;      //最优买入全价
	int64_t     iAskTotalPrice;      //最优卖出全价
	int			nBidQuoteTime;		 //买入报价时间
	int			nAskQuoteTime;		 //卖出报价时间
};


struct TD_HQBufferData
{
	int		nDataLen;
	char	pBuffer[4];
};
...
#define ID_TDFDATATYPE_NBSTEP			20	//NBSTEP 新债券逐笔成交
#define ID_TDFDATATYPE_NBORDER			21	//NBORDER 新债券逐笔委托
#define ID_TDFDATATYPE_BBQSNAP			22	//BBQSnap BBQ债券快照数据
#define ID_TDFDATATYPE_BBQSTEP			23	//BBQSTEP BBQ债券逐笔成交
#define ID_TDFDATATYPE_FIBONDDATA		24	//FIBOND 固定收益债券行情
#define ID_TDFDATATYPE_HQBUFFER			25	//HQ BUFFER数据
...
struct TDFDynData_DataFeed_Packet
{
    ...
    void *pData;                 // 其他数据类型
	int	 nFlag;					 // nFlag为1时且pData非NULL时，pData需要释放(delete), 其它情况下不需要
};
...
struct TDBar_Day_Packet
{
	...
	union TDBar_DayEx_Packet
	{
		...
		{
			int64_t nSettlement;	//结算价
			int64_t nPreSettlement;	//前结算价
			int nPosition;		//合约持仓量			
			int nPrePosition;	//前持仓量
		}Futures;
	}ExtData;

	union TDBar_DayEx_Packet_V1
	{
		struct indexdata
		{
			int nStocks;		//品种个数
